...
if (isset($_POST['id_tarjeta'])) {
    ...
    if (mysqli_query($conexion, $queryEliminar)) {
        echo "<script>alert('Tarjeta eliminada correctamente.'); window.location.href='usuario_elimina_tarjeta.php';</script>";
    } else {
        echo "<script>alert('Error al eliminar la tarjeta.'); window.location.href='usuario_elimina_tarjeta.php';</script>";
    }
} else {
    header('Location: usuario_elimina_tarjeta.php');
}
?>
