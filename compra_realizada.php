...
if ($conexion->query($sql_insert_pedido) === TRUE) {
    ...
    $sql_insert_juegos_pedido = "INSERT INTO juegos_pedido (id_pedido, id_juego) 
                                 SELECT $id_pedido, id_juego FROM carrito WHERE id_usuario = $id_usuario";
    if (!$conexion->query($sql_insert_juegos_pedido) === TRUE) {
        echo "Error al asociar juegos del carrito al pedido: " . $conexion->error . "<br>";
    }
} else {
